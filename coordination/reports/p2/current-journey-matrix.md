...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| P2-J01 | `PASS` | Visitor opens `/`, reaches login, and can use the skip link and shell navigation | `npm run test:p2:e2e` | `tests/p2/browser/core-journeys.spec.ts`; `tests/p2/browser/accessibility.spec.ts` | No local automated failure | S01/S11 | Re-run unchanged candidate on isolated staging | Yes if regressed |
...
| P2-J08 | `BLOCKED_ENV` | Teacher creates groups; members see only their group; simultaneous messages never cross groups | `npm run test:p2:load` plus staging browser matrix | `coordination/reports/p2/current-load.md` | Independent `uais-staging`, isolated database, accounts, and cleanup proof are absent | S11/S14/S22 | Provision isolated staging, enable groups there only, then execute matrix | Yes |
