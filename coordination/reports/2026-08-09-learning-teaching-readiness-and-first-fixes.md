...
| **B2** external storage env | **Code side resolved** — the storage trio (`UAIS_TEACHING_COURSE_MANAGEMENT_BACKEND`, `UAIS_EXTERNAL_STORAGE_BASE_URL`, `UAIS_EXTERNAL_STORAGE_ACCESS_TOKEN`) is promoted out of quarantined-legacy into `active-production` with `productionDefault: required`, across the catalog, `docs/env-surface.md` and `.env.local.example`; the tier cap moved 21 → 24 with the reason pinned in `tests/env-surface.test.ts`. Setting the values in the deployed lane is the operator step that remains; `npm run release:chatroom-readiness` verifies it. |
| **B3** transcript schema v2 | **Resolved** — the storage service now DECLARES its chatroom schema support on `/healthz` (`learningChatroomStorageSchema`, transcripts v2 accepting v1, shares v1). Compatibility is settled before the first write instead of discovered as a rejected round, and a service built before v2 has no such field, which is itself the signal. `release:chatroom-readiness --probe` reads the declaration; verified live in both directions against the real route. |
| **B4** flag parity | **Resolved as a failure mode** — a set-but-not-`on` value no longer fails silently: the single reader logs a structured warning once per distinct wrong value per process, naming the configured and expected values, and the preflight blocks on it before a flip. Setting the literal `on` remains the operator step. |
