...
const loginCopy = {
  "zh-CN": {
    ...
    accountLogin: "账号密码登录",
    accountLabel: "账号",
    accountPlaceholder: "教师账号或受邀学生账号",
    passwordLabel: "密码",
  ...
  "en-US": {
    ...
    accountPlaceholder: "Teacher account or invited student account",
  ...
  const handleSubmit = useCallback(
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        if (!response.ok) {
          setError(response.status === 401 ? t.invalidError : (result?.error ?? t.serverError));
          return;
        }
    ...
    [account, password, router, t.emptyError, t.invalidError, t.serverError],
  );
        ...
        <main className="relative flex min-h-[100dvh] items-center px-5 py-8 sm:px-8 lg:px-10 xl:px-16">
          ...
          <div className="mx-auto w-full max-w-[560px] pt-16 lg:pt-0">
            ...
            <form onSubmit={handleSubmit} className="mt-7 space-y-5" noValidate>
              ...
              {error ? (
                <p className="rounded-lg border border-[#f0b7c9] bg-[#fff1f5] px-4 py-3 text-sm font-semibold text-[#a12f56] dark:border-[#89435b] dark:bg-[#321420] dark:text-[#ffb7ca]">
                  {error}
                </p>
              ) : null}
