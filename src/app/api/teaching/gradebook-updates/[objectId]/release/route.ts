...
import {
  ...
  releaseTeachingGradebookUpdate,
  resolveTeachingOperationDataDir,
  TeachingOperationStoreError,
  type TeachingGradebookReleaseProviderReceipt,
...
type TeachingGradebookReleaseRouteContext = {
  params: { objectId: string } | Promise<{ objectId: string }>;
};

type TeachingGradebookReleasePostHandlerDeps = {
...
export const POST = createTeachingGradebookReleasePostHandler();

export function createTeachingGradebookReleasePostHandler(
  deps: TeachingGradebookReleasePostHandlerDeps = {},
) {
  ...
  return async function POST(request: Request, context: TeachingGradebookReleaseRouteContext) {
    ...
    try {
      ...
      if (
        ...
        throw new TeachingOperationStoreError(
          503,
          "Production teaching gradebook release requires external teaching operations storage.",
        );
      }
      ...
      const externalAudit = readExternalTeachingOperationAudit
        ...
        : undefined;
      const pendingUpdate = externalAudit
      ...
      if (!readOwnership) {
        return jsonResponse(403, {
          ...
          traceId,
          access: createDeniedAccess("teacher-course-ownership-required", pendingUpdate.courseId),
          redaction: createRedaction(),
        }, traceId);
      }
      ...
      if (
        ...
        !new Set(ownership.courseIds ?? []).has(pendingUpdate.courseId)
      ) {
        return jsonResponse(403, {
          error: "UAIS teaching operation course ownership is required.",
          traceId,
          access: createDeniedAccess("course-scope-denied", pendingUpdate.courseId),
          redaction: createRedaction(),
        }, traceId);
      }

      const releaseInput = {
        objectId: pendingUpdate.objectId,
      ...
      const providerRelease = await syncGradebookReleaseProvider({
        env,
        fetch: deps.fetch ?? fetch,
        gradebookUpdate: pendingUpdate,
        actorId: releaseInput.actorId,
        traceId,
        releasedAt: releaseInput.now.toISOString(),
      });
      const { gradebookUpdate, notification, receipt } =
        appendExternalTeachingOperation && externalAudit
          ? await releaseExternalTeachingGradebookUpdate({
              gradebookUpdate: pendingUpdate,
              actorId: releaseInput.actorId,
              audit: releaseInput.audit,
              appendExternalTeachingOperation,
              providerRelease,
              now: releaseInput.now,
            })
          : await releaseTeachingGradebookUpdate({
              dataDir,
              objectId: releaseInput.objectId,
              actorId: releaseInput.actorId,
              audit: releaseInput.audit,
              providerRelease,
              now: releaseInput.now,
...
async function syncGradebookReleaseProvider(input: {
  env: Record<string, string | undefined>;
  fetch: typeof fetch;
  ...
  if (provider !== "external") {
    throw new TeachingOperationStoreError(
      503,
      "Gradebook release provider is not production-ready.",
    );
  }
  ...
  const normalizedProviderUrl = readExternalGradebookProviderUrl(
    providerUrl,
    "Gradebook release provider URL is invalid.",
    input.env,
  );
  ...
  const response = await input.fetch(normalizedProviderUrl, {
    ...
    body: JSON.stringify({
      ...
      releasePolicy: input.gradebookUpdate.releasePolicy,
  ...
  if (!response.ok) {
    throw new TeachingOperationStoreError(502, "Gradebook release provider sync failed.");
  }

  const providerReceipt = await response.json().catch(() => undefined);
  if (!isRecord(providerReceipt) || providerReceipt.status !== "released") {
    throw new TeachingOperationStoreError(502, "Gradebook release provider sync failed.");
  }

  return {
    providerStatus: "gradebook-provider-released",
    providerReleaseId: requireSafeProviderId(providerReceipt.releaseId, "provider release id"),
    providerReleasedAt: input.releasedAt,
  };
}
...
function createErrorResponse(error: unknown, traceId: string) {
  ...
  return jsonResponse(500, {
    error: "Teaching gradebook release request failed.",
    traceId,
    redaction: createRedaction(),
  }, traceId);
}
